// module-11-rag-mcp/lesson-11-4-retriever/store.go
...
// memStore — хранилище векторов в памяти. Реализует сразу два интерфейса Eino:
// indexer.Indexer (складывать документы) и retriever.Retriever (искать похожие).
...
// Шаги: эмбеддим запрос, считаем близость к каждому документу, берём top-K.
func (m *memStore) Retrieve(ctx context.Context, query string, _ ...retriever.Option) ([]*schema.Document, error) {
	...
	// Сортируем по убыванию близости — самые похожие в начале.
	sort.Slice(ranked, func(i, j int) bool { return ranked[i].score > ranked[j].score })

	n := min(topK, len(ranked))
	out := make([]*schema.Document, n)
	for i := range out {
		out[i] = ranked[i].doc.WithScore(ranked[i].score) // кладём близость в Score документа
	}
	return out, nil
}
