// minicode/stage-07-ui/main.go

// Mini Code — мини-агент-кодер на Go (Eino + Ollama). Этап 7: интерфейс.
// Берём финального агента из этапа 6 (трейсинг + флаги + режимы) и надеваем на
// него терминальный интерфейс (TUI) в духе Claude Code — это отдельный слой,
// ядро агента не меняется. Режимы запуска:
//   - REPL (по умолчанию) — диалог в обычной консоли;
//   - одноразовый -q "вопрос" — ответил и вышел (для скриптов и CI);
//   - -ui — терминальный интерфейс: рамка-приветствие, поле ввода, спиннер.
//
// Запуск из папки модуля:
//
//	go mod tidy
//	go run .                          # интерактивный REPL
//	go run . -ui                      # терминальный интерфейс (TUI)
...
type config struct {
	...
	ui    bool   // запустить терминальный интерфейс (TUI) вместо REPL
}
...
func parseConfig() config {
	...
	flag.BoolVar(&c.ui, "ui", false, "терминальный интерфейс (TUI) в духе Claude Code вместо REPL")
	flag.Parse()
	return c
}
...
func main() {
	...
	// Режим терминального интерфейса (TUI) в духе Claude Code.
	if cfg.ui {
		if err := runTUI(ctx, agent); err != nil {
			log.Printf("ошибка интерфейса: %v", err)
		}
		return
	}
